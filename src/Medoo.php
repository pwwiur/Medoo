<?php
	class database{
		...
		public static function get($table, $join = NULL, $columns = NULL, $where = NULL){
			$column = $where == NULL ? $join : $columns;
			$is_single_column = (is_string($column) && $column !== '*');
			$query = self::query(self::select_context($table, $join, $columns, $where) . ' LIMIT 1');
			if ($query){
				...
				if (isset($data[ 0 ])){
					...
					foreach ($columns as $key => $value){
						if (is_array($value)){
							self::data_map(0, $key, $value, $data[ 0 ], $stack);
						}
						else{
							self::data_map(0, $key, preg_replace('/^[\w]*\./i', "", $value), $data[ 0 ], $stack);
						}
					}
					return $stack[ 0 ];
				}
